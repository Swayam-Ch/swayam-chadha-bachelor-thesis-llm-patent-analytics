...
fetch_claims.py
===============
Downloads PatentsView bulk claims files (2010-2023) and extracts the first
independent claim (claim_sequence = 1) for every patent in the AIPD universe.

Handles two ID formats present in the AIPD dataset:
  - Granted patents   : 6-8 digit numbers (e.g. 7836071)  → g_claims_YYYY.tsv.zip
  - Pre-grant publs.  : 11-digit numbers  (e.g. 20100157136) → pg_claims_YYYY.tsv.zip
  - Reissue patents   : RE-prefixed (e.g. RE41071)         → g_claims (treated as granted)

Outputs:
  ~/Downloads/claims_claim1.csv   — patent_id, claim1_text, claim_type (granted/pregrant)

Usage:
  python fetch_claims.py

Notes:
  - Each year's file is ~3 GB unzipped. The script streams line-by-line so
    memory usage stays low (~500 MB peak).
  - Total download: ~6 GB compressed over 28 files. Allow 1-2 hours on a
    standard connection.
  - PatentsView is migrating to USPTO's Open Data Portal on 20 March 2026.
    If downloads fail, check: https://data.uspto.gov/bulkdata
"""
